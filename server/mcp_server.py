...
2. web_search_github("电商") → fallback 搜索 (DuckDuckGo, 免费无限)
...
mcp = FastMCP(
    "github-code-rag",
    instructions="""# 需求分析 Agent — 先搜再问

## 角色
你是一位资深需求分析师 + 代码研究员。用户提出想法后，先搜 GitHub 上类似项目，再基于搜索结果向用户提问，逐步明确需求。

## 核心工作流

### Step 1: 先搜再看（GitHub）
**不要直接开始提问！严格按照以下顺序执行，不允许跳过任何步骤：**
...
5. 如果 search_github 限流 → **web_search_github("XX")** — 用 Bing 搜 GitHub（免费无限）
...
## 关键规则
1. **先搜再问** — 永远先搜 GitHub，不要凭空提问
2. **必须先调 search_history** — 用户提出需求后，第一步必须是 search_history，不允许跳过
...
def web_search_github(query: str, limit: int = 10) -> str:
    """通过 Bing 搜索 GitHub 上的开源项目（免费，无需 API Key，无限额度）。
    ...
    url = f"https://cn.bing.com/search?q={urllib.parse.quote(site_query)}&count={limit}"
    ...
    # Parse Bing results
    results = []
    # Find all result blocks: <li class="b_algo">
    ...
    if not results:
        return f"没有找到匹配的 GitHub 项目: {query}"
    ...
    try:
        if _engine is None:
            ...
            _init_engine(data_dir)
        for r in results:
            repo_name = r["href"].replace("https://github.com/", "").strip("/").split("/")
            if len(repo_name) >= 2:
                name = f"{repo_name[0]}/{repo_name[1]}"
                _engine.record_search(query, RepoInfo(
                    name=name, url=r["href"], language="",
                    indexed_at="", description=r["snippet"][:200] or "",
                    stars=0, file_count=0,
                ))
    except Exception:
        pass  # Best-effort

    output = [f"Bing 搜索结果 (site:github.com): {query}\n"]
    for i, r in enumerate(results, 1):
        title = r["title"].replace("GitHub - ", "").replace("github - ", "")
