<div align="center"><pre>
  ...
  <p align="center">
    <a href="https://pypi.org/project/github-code-rag/"><img
  src="https://img.shields.io/pypi/v/github-code-rag?style=flat-square" alt="PyPI" /></a>
  ...
  **复用优先**是写在系统提示词里的铁律：
  ...
  ▎ 所有支持 MCP 协议的客户端都能用。如果你用的客户端不在上面列表里，配置方式基本一样 —— 把 command 指向 github-code-rag
  ▎  即可。
  ...
  支持哪些 MCP 客户端？
  所有支持 MCP 协议的 —— Claude Code、Claude Desktop、Cursor、Windsurf、Cline、Codex、Gemini CLI、OpenCode… 只要能配 MCP
   server 就能用。
  ...
  会加向量搜索吗？
  可能，但不是当前优先级。目前的经验是：先搜对仓库 > 在仓库里精确搜 >
  语义搜。向量搜索在代码场景下的实际收益没有营销文案说的那么大。如果你有强烈需求，欢迎提 Issue。
...
## English
  ...
  <p align="center">
    ...
    <br/>
    <em>github-code-rag makes your AI research first, ask questions second, and only then build — like a real senior
  engineer.</em>
  </p>
  ...
  Once installed, your Claude / Cursor / Codex won't jump straight into writing code. It searches GitHub for similar
  projects first, asks clarifying questions based on real-world examples, confirms requirements, and then — and only
  then — starts building, **reusing code from repos it has read**.
  ...
  **The problem is the workflow.** Most AI coding follows a broken process:
  ...
  Real engineers don't work that way:
  ...
  **github-code-rag puts "engineer thinking" inside your MCP.**
  ...
  The system prompt hardcodes the workflow. The AI *must* search GitHub before asking questions. Skipping steps? The
  tool design makes "write first, search never" harder than doing it right.
  ...
  #### Code reuse, enforced — Stop reinventing the wheel

  Everything you read gets indexed locally. When it's time to write code, search and reuse.
  **Reuse-first** is a hard rule in the system prompt:

  > "If you can find an existing implementation, don't write it from scratch. Violations = invalid output."

  Reused code is auto-attributed:
  ...
  Every search is saved to local SQLite. Research "e-commerce app" last week? Pick up right where you left off next
  time. No re-searching.

  → search_history("e-commerce app")
    Projects read in category 'e-commerce app':
      - justjavac/wechat-app-eshop (JavaScript) @ 2026-03-15
      - xxx/shop-wxapp (TypeScript) @ 2026-03-14
    → Records found, use search_code for specific implementations

  GitHub API + Bing fallback — Never get rate-limited

  - GitHub Search API: high quality, sorted by stars (60 req/hr free, 5000/hr with token)
  - Bing search fallback: free, unlimited, no API key needed
  - Auto-degrades on rate limits, no manual intervention

  Zero git clone · Zero vector DB · 1 dependency total

  - All via GitHub REST API — no cloning repos to your machine
  - SQLite + FTS5 full-text index — no vector database, no embedding model
  - Runtime dependency: only mcp>=1.0, everything else is Python stdlib
  - Startup < 1s, memory footprint < 50MB
  ...
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ AI starts writing immediately │          Full speed ahead           │          Searches GitHub first           │
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ Requirement understanding     │              Guessing               │ Confirmed iteratively with real projects │
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ Code quality                  │        Written from scratch         │      Reused from 20M+ GitHub repos       │
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ Mistakes                      │ Every pitfall, learned the hard way │     Skip what others already solved      │
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ Start from zero every time    │            Always fresh             │     Accumulates knowledge over time      │
  ├───────────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────┤
  │ Source attribution            │    Who knows where it came from     │ Auto-tagged # Source: owner/repo/file.py │
  └───────────────────────────────┴─────────────────────────────────────┴──────────────────────────────────────────┘
  ...
  2. Configure a Token (optional but recommended)

  export GITHUB_TOKEN=ghp_your_token_here

  Works without a token too — built-in Bing fallback is free and unlimited.
  ...
  ▎ Not sure where the binary is? Run which github-code-rag (macOS/Linux) or where github-code-rag (Windows).
  ...
  Settings → MCP Servers → Add MCP Server, stdio mode, command:
  ...
  Settings → MCP Servers → Add new MCP Server → Local executable:
  ...
  Add to the mcp servers section of your OpenCode config:
  ...
  ▎ Works with any MCP-compatible client. If yours isn't listed, the pattern is the same — point the command at
  ▎ github-code-rag.
  ...
  │       Tool        │                         Description                         │
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  │ search_history    │ Query search history for a category (always the first step) │
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  │ search_github     │ Search repos via GitHub API, sorted by stars                │
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  │ web_search_github │ Bing search fallback, free and unlimited                    │
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  ...
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  │ index_status      │ Check local index stats                                     │
  ├───────────────────┼─────────────────────────────────────────────────────────────┤
  ...
  How It Works
    ...
    │                                                           │
    │   ┌─────────────────────────────────────────────────┐   │
    │   │  System Prompt (Requirements Agent Playbook)    │   │
    │   │     · Search first · Iterate · Reuse-first      │   │
    │   │     · One question at a time · Source tags      │   │
    │   └───────────────────────┬─────────────────────────┘   │
    │                           │ Guides how AI uses tools    │
    │   ┌───────────────────────▼─────────────────────────┐   │
    │   │  9 MCP Tools                                    │   │
    ...
    │  ┌────────────▼───────────┐   ┌───────────▼──────────┐  │
    │  │  GitHub REST API       │   │  SQLite + FTS5       │  │
    │  │  + Bing fallback       │   │  Local code index    │  │
    │  │  Zero git clone        │   │  Trigram search      │  │
    │  └────────────────────────┘   └──────────────────────┘  │
    │                                                           │
    └───────────────────────────────────────────────────────────┘
  ...
  ├──────────────────────────────┼──────────────────────────┼─────────────────────────────┤
  │ "How is auth implemented?"   │            No            │             Yes             │
  ├──────────────────────────────┼──────────────────────────┼─────────────────────────────┤
  │ Extra dependencies           │ Zero (built into SQLite) │ Vector DB + embedding model │
  ├──────────────────────────────┼──────────────────────────┼─────────────────────────────┤
  ...
  Our approach: two-stage search.
  First, use GitHub Search to find the right repos (which projects are worth referencing).
  Then, use FTS5 for precise code search within those repos (where is the actual implementation).
  ...
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Requirements Agent guidance │       Yes       │        No         │          No          │         No          │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Reuse-first methodology     │       Yes       │        No         │          No          │         No          │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Search history / categories │       Yes       │        No         │          No          │         No          │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Search public GitHub repos  │       Yes       │  No (local only)  │ Yes (requires clone) │         Yes         │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Local code indexing         │   Yes (FTS5)    │ Yes (Zig native)  │ Yes (FTS5 + vector)  │         No          │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Zero git clone              │       Yes       │        N/A        │          No          │         Yes         │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Free Bing fallback          │       Yes       │        No         │          No          │         No          │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ External dependencies       │     1 (mcp)     │ 0 (single binary) │  Many (Bun + ONNX)   │        Many         │
  ├─────────────────────────────┼─────────────────┼───────────────────┼──────────────────────┼─────────────────────┤
  │ Startup time                │      < 1s       │     Very fast     │        Slower        │        Fast         │
  └─────────────────────────────┴─────────────────┴───────────────────┴──────────────────────┴─────────────────────┘

  One-line summary:
  - codedb / codebase-rag = faster, better code search tools
  - github-code-rag = requirements analysis Agent with a methodology + code search

  Anyone can build a tool. Methodology + tool is the moat.
  ...
  All MCP-compatible clients — Claude Code, Claude Desktop, Cursor, Windsurf, Cline, Codex, Gemini CLI, OpenCode… If it
  can configure an MCP server, it works.

  Does my code get uploaded anywhere?
  No. All indexes are stored in a local SQLite database. GitHub API is only used to read public repos. Your code never
  leaves your machine.

  Why not just use the gh CLI?
  Because MCP tools should be transparent to the AI — it shouldn't need to know what CLI tools you have installed. Also,
   pure Python implementation means zero system dependencies.

  Will you add vector search?
  Maybe, but it's not a current priority. Our experience so far: find the right repo first → precise code search within
  that repo → semantic search is a distant third. The real-world benefits of vector search for code are often overstated
   in marketing. If you feel strongly about it, open an issue.
  ...
  - Find it useful → star the project so more people find it
